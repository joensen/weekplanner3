...
- `GET /api/system-status` - Current Pi/watchdog network status for on-screen warnings
...
## Troubleshooting
Inspect the Wi-Fi watchdog log with:

```bash
sudo tail -n 200 /var/log/wifi-watchdog.log
```

If the weekplanner should show live Pi network issues on screen, keep the watchdog log readable by the app and point `.env` at it with `WIFI_WATCHDOG_LOG_FILE=/var/log/wifi-watchdog.log`.

If you want to implement auto-recovery later, extend the watchdog so that after a few consecutive failed checks it restarts the Wi-Fi connection or reboots the device as a last resort.
